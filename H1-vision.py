...
def detect_route_interface(robot_ip: str, log_func=None) -> str:
    """
    自动找出访问 robot_ip 时系统实际走的本机网卡。

    在 Linux 上优先使用：
        ip route get 192.168.123.162
    例如输出里会有：
        192.168.123.162 dev wlan0 src 192.168.123.xxx ...
    此时 DDS 应绑定 wlan0，而不是旧的有线网卡 enx...
    """
    robot_ip = (robot_ip or "").strip()
    ...
    if robot_ip:
        try:
            ...
            if log_func:
                log_func(f"自动检测网卡：ip route get {robot_ip} 未成功，将尝试 SDK 默认接口。原因：{exc}")
...
def resolve_dds_interface(robot_ip: str, requested_iface: str, log_func=None) -> str:
    ...
    if requested_key in _AUTO_IFACE_VALUES:
        if route_iface:
            if log_func:
                log_func(f"自动检测到访问机器人 {robot_ip} 使用的本机网卡：{route_iface}")
            return route_iface
...
def ensure_dds_initialized(iface: str, log_func=None, robot_ip: str = "") -> None:
    """
    ...
    requested_iface = (iface or "").strip()
    iface = resolve_dds_interface(robot_ip, requested_iface, log_func)

    if _DDS_FACTORY_INITIALIZED:
        if log_func and iface != _DDS_FACTORY_IFACE:
            log_func(
                f"DDS 已初始化过，当前继续使用首次网卡：{_DDS_FACTORY_IFACE or 'SDK 默认接口'}，"
                f"忽略新的网卡：{iface or 'SDK 默认接口'}。如需切换有线/无线网卡，请重启程序后再连接。"
            )
        return

    if iface:
        if log_func:
            log_func(f"初始化 Unitree DDS，绑定本机网卡：{iface}")
...
class LowStateWorker(QThread):
    ...
    def _run_sdk2(self) -> None:
        try:
            ...
            ensure_dds_initialized(iface, self.log_signal.emit, self.config.robot_ip)
...
class H1RobotClient(QObject):
    ...
    def connect_robot(self, config: RobotConfig) -> None:
        ...
        self.log_signal.emit(
            ...
            f"iface={config.network_interface or 'auto'}, topic={config.lowstate_topic}, idl={config.lowstate_idl}"
        )
...
class MainWindow(QMainWindow):
    ...
    def _build_replay_report_tab(self) -> QWidget:
        ...
        heading = QLabel("录像回放与事件时间轴")
        heading.setObjectName("LiveTitle")
        intro = QLabel("集中管理录像、演示事件。")
        intro.setObjectName("LiveSubtitle")
