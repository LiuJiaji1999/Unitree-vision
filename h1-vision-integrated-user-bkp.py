...
#   X、Y 表示平面位置，Z 表示高度。颜色表示激光回波强度，也就是雷达打到物体表面后返回信号的强弱。
...
class MainWindow(QMainWindow):
    ...
    def _build_camera_tab(self) -> QWidget:
        ...
        self.start_camera_btn = QPushButton("启动机器人相机并显示")
        self.stop_camera_btn = QPushButton("停止显示")
        self.stop_remote_camera_btn = QPushButton("停止机器人相机服务")
        ...
        self.camera_status_label = QLabel("未启动。点击“启动机器人相机并显示”后，会先 SSH 启动机器人端服务，再在此处显示画面。")
    ...
    def _on_realsense_start_finished(self, exit_code: int, exit_status) -> None:
        ...
        self.start_camera_btn.setText("启动机器人相机并显示")
        if exit_code != 0:
